...
class QuizCubit extends Cubit<QuizState> {
      ...
      for (var quiz in quizzes) {
        ...
        if (!categoryExist) {
          quiz.category = Category(
              name: 'Unknown', gradient: CColors.greenGradientColor, id: '0');
        }
      }
  ...
  void changeImage(String path) {
    if (state is QuizChanged) {
      ...
      emit(QuizChanged(
          quizzes: state.quizzes,
          categories: state.categories,
          newQuiz: Quiz.empty(id: '${DateTime.now().millisecondsSinceEpoch}')));
    }
  }
  ...
  void multipleQuestionCreate() {
    if (state is QuizChanged) {
      state.newQuiz!.questions
          .add(MultipleChoiceQuestion.empty(options: [''], correctAnswers: []));
      emit(QuizChanged(
          quizzes: state.quizzes,
          categories: state.categories,
          newQuiz: state.newQuiz!));
  ...
  void selectOption(int questionIndex, int optionIndex) {
    if (state is QuizChanged) {
      if (state.newQuiz!.questions[questionIndex] is OneChoiceQuestion) {
        (state.newQuiz!.questions[questionIndex] as OneChoiceQuestion)
            .correctAnswer = optionIndex;
      } else {
        if ((state.newQuiz!.questions[questionIndex] as MultipleChoiceQuestion)
            .correctAnswers
            .contains(optionIndex)) {
          (state.newQuiz!.questions[questionIndex] as MultipleChoiceQuestion)
              .correctAnswers
              .remove(optionIndex);
        } else {
          (state.newQuiz!.questions[questionIndex] as MultipleChoiceQuestion)
  ...
  bool isSelected(int questionIndex, int optionIndex) {
    if (state is QuizChanged) {
      if (state.newQuiz!.questions[questionIndex] is OneChoiceQuestion) {
        return (state.newQuiz!.questions[questionIndex] as OneChoiceQuestion)
                .correctAnswer ==
            optionIndex;
      } else {
  ...
  void startQuiz(int indexQuiz) {
    ...
    for (int i = 0; i < state.quizzes[indexQuiz].questions.length; i++) {
      answers[i] = List.generate(
          state.quizzes[indexQuiz].questions[i].options.length,
          (index) => false);
    }
    emit(QuizTaken(
        quizzes: state.quizzes,
  ...
  String checkBoxType(int quizIndex, int optionIndex, int questionIndex) {
    if (state.quizzes[quizIndex].questions[questionIndex]
        is OneChoiceQuestion) {
      if ((state.quizzes[quizIndex].questions[questionIndex]
                  as OneChoiceQuestion)
              .correctAnswer ==
          optionIndex) {
        return 'green';
  ...
  void loadCategories() async {
    ...
    emit(QuizChanged(
        quizzes: state.quizzes, categories: cat, newQuiz: state.newQuiz));
  }
