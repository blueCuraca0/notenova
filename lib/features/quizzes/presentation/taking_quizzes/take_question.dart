...
class TakeQuestion extends StatelessWidget{
  ...
  const TakeQuestion({super.key, required this.question, required this.indexQuestion, required this.percentage, required this.quiz});
                            ...
                            CustomButton(text: LocaleKeys.next.tr(), onPressed: (){
                              if (isLastQuestion(context, indexQuestion)){
                                Navigator.push(context, MaterialPageRoute(
                                    builder: (context) =>
                                        CheckAnswers(indexQuiz: getQuizIndex(context, quiz))));
                              }
                              else {
                                Navigator.push(context, MaterialPageRoute(
                                    builder: (context) =>
                                        TakeQuestion(question: getNextQuestion(
                                            context, indexQuestion + 1),
                                          indexQuestion: indexQuestion + 1,
                                          percentage: getPercentage(
                                              context, indexQuestion + 1), quiz: quiz,)));
                              }
                              },
